        ...
        <div className="border-t border-chrome dark:border-border-dark">
          <div className="flex items-center gap-2 p-2 overflow-x-auto no-scrollbar">
            ...
            {activeTool === Tool.Text && (
              <>
                <button onMouseDown={(e) => { e.preventDefault(); handleTextCommand('bold'); }} className={`p-3 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors shrink-0 text-charcoal dark:text-text-dark`}>{ICONS.bold}</button>
                <button onMouseDown={(e) => { e.preventDefault(); handleTextCommand('italic'); }} className={`p-3 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors shrink-0 text-charcoal dark:text-text-dark`}>{ICONS.italic}</button>
                <button onMouseDown={(e) => { e.preventDefault(); handleTextCommand('strikeThrough'); }} className={`p-3 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors shrink-0 text-charcoal dark:text-text-dark`}>{ICONS.strikethrough}</button>
                <button ref={textColorButtonRef} onMouseDown={(e) => { e.preventDefault(); setIsTextColorPopoverOpen(p => !p); }} className={`p-3 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors shrink-0 ${isTextColorPopoverOpen ? 'bg-accent/10 text-accent dark:bg-accent/20' : 'text-charcoal dark:text-text-dark'}`}>{ICONS.textColor}</button>
                <button onMouseDown={(e) => { e.preventDefault(); handleTextCommand('insertUnorderedList'); }} className={`p-3 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors shrink-0 text-charcoal dark:text-text-dark`}>{ICONS.bulletList}</button>
                
                <div className="h-7 w-px bg-chrome dark:bg-border-dark shrink-0"></div>
              </>
            )}
            
            <button onClick={() => imageInputRef.current?.click()} className="p-3 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors shrink-0 text-charcoal dark:text-text-dark" aria-label="Add image">{ICONS.image}</button>
            <input type="file" accept="image/*" ref={imageInputRef} onChange={handleImageUpload} className="hidden" />
            
            <button onClick={isRecording ? stopRecording : startRecording} className={`p-3 rounded-full transition-colors shrink-0 ${isRecording ? 'text-coral pulse-rec-animation' : 'hover:bg-black/10 dark:hover:bg-white/10 text-charcoal dark:text-text-dark'}`} aria-label={isRecording ? 'Stop recording' : 'Start recording'}>
